...
cmd_opt = argparse.ArgumentParser(description='Argparser for fp gan', allow_abbrev=False)
...
cmd_opt.add_argument('-ent_lam', type=float, default=1.0, help='lambda for entropy')
...
cmd_opt.add_argument('-kde_sigma', type=float, default=0.1, help='kernel bandwidth of kde')
cmd_opt.add_argument('-mmd_bd', type=float, default=0.1, help='kernel bandwidth of mmd')
cmd_opt.add_argument('-gp_lambda', type=float, default=0, help='use gradient penalty')
...
cmd_opt.add_argument('-g_iter', type=int, default=5, help='iters of generator update')
cmd_opt.add_argument('-grad_clip', type=int, default=5, help='clip of gradient')
cmd_opt.add_argument('-energy_type', type=str, default='mlp', help='type for energy func')
cmd_opt.add_argument('-num_landmarks', type=int, default=1000, help='number of landmarks')

# args for gaussian experiment
cmd_opt.add_argument('-gauss_dim', type=int, default=1, help='dimension of gaussian')
...
cmd_opt.add_argument('-data_dump', type=str, default=None, help='synthetic data dump')
cmd_opt.add_argument('-data_name', type=str, default=None, help='synthetic data name')

# args for generator
cmd_opt.add_argument('-flow_type', type=str, default='norm', help='type for flows (norm || iaf)')
cmd_opt.add_argument('-num_flows', type=int, default=1, help='number of flows in the mixture model')
...
cmd_opt.add_argument('-sp_iters', type=int, default=0, help='spectrum norm iters')

# args for hmc
cmd_opt.add_argument('-mcmc_type', type=str, default='None', help='type for mcmc', 
    choices=['None', 'HMC', 'GeneralHmc', 'ResGeneralHmc', 'SGLD'])
cmd_opt.add_argument('-use_mh', type=eval, default=True, help='use rejection sampling?', choices=[True, False])
cmd_opt.add_argument('-mcmc_steps', type=int, default=1, help='number of mcmc steps')
cmd_opt.add_argument('-hmc_inner_steps', type=int, default=1, help='number of hmc inner steps')
cmd_opt.add_argument('-use_2nd_order_grad', type=eval, default=True, choices=[True, False])
cmd_opt.add_argument('-clip_samples', type=eval, default=False, choices=[True, False])

# args for sgld
cmd_opt.add_argument('-sgld_noise_std', type=float, default=1.0, help='std of injected noise for LD update')
cmd_opt.add_argument('-sgld_clip_value', type=float, default=1.0, help='clip value for gradient in LD update')
cmd_opt.add_argument('-sgld_clip_mode', type=str, default='norm', help='type of gradient clipping in LD update', choices=['norm', 'value'])
cmd_opt.add_argument('-moment_penalty', type=float, default=0, help='coefficient for norm of momentums in hmc')
cmd_opt.add_argument('-hmc_adaptive_mode', type=str, default='human', help='adaptive tuning for hmc', choices=['auto', 'human', 'none'])

cmd_opt.add_argument('-hmc_step_size', type=float, default=1e-2, help='hmc step size')
cmd_opt.add_argument('-hmc_p_sigma', type=float, default=1.0, help='momentum p ~ N(0, sigma)')
...
cmd_opt.add_argument('-test_batch_size', type=int, default=100, help='bsize test')
...
cmd_opt.add_argument('-net_type', type=str, default='mlp', help='type for vae (cnn || mlp)')
