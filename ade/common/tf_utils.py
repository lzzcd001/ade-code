...
from ade.common.custom_adam import CustomizedAdamOptimizer as tf_optimizer
# from tensorflow.train import AdamOptimizer as tf_optimizer
...
class MLP(snt.AbstractModule):
    def __init__(self, input_dim, hidden_dim, depth, output_dim=1, act_hidden=tf.nn.relu, sp_iters=0, vars=None, name='mlp'):
    ...
    def spectral_norm(self, w, l, iteration=1):
        '''
        ...
        u = tf.get_variable("u-%d" % l, [1, w_shape[-1]], initializer=tf.random_normal_initializer(), trainable=False)
    ...
    def _build(self, x):
        ...
        for l in range(len(self.list_linear)):
            ...
            w = self.spectral_norm(w, l, self.sp_iters)
            h = tf.matmul(h, w) + b
